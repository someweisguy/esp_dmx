...
typedef enum {
  ...
  DMX_DISCOVERY_COMMAND = 0x10,           // The packet is an RDM discovery command.
  DMX_DISCOVERY_COMMAND_RESPONSE = 0x11,  // The packet is a response to an RDM discovery command.
  DMX_GET_COMMAND = 0x20,                 // The packet is an RDM get request.
  DMX_GET_COMMAND_RESPONSE = 0x21,        // The packet is a response to an RDM get request.
  DMX_SET_COMMAND = 0x30,                 // The packet is an RDM set request.
  DMX_SET_COMMAND_RESPONSE = 0x31,        // The packet is a response to an RDM set request.
} dmx_cc_t;

typedef enum {
  DMX_RESPONSE_TYPE_ACK = 0x00,
  DMX_RESPONSE_TYPE_ACK_TIMER = 0x01,
  DMX_RESPONSE_TYPE_NACK_REASON = 0x02,
  DMX_RESPONSE_TYPE_ACK_OVERFLOW = 0x03
} dmx_response_type_t;
